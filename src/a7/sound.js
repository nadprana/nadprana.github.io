...
class SoundBar {
 	...
 	getAOFValue(index, count, offset=0) {
 		...
 		let col = Math.floor(this.values.length*(index*5)/count)
 	...
 	// Set the soundbar to the current audio
 	update() {
 		this.avg = 0
		let lastFreq = 0
		for (var i = 0; i < this.values.length; i++) {
...
function getMIDIMessage(midiMessage) {
   ...
   for (var i = 0; i < population.individuals.length; i++) {
    	 ...
    	 // let aof = controls.selectedAOF
	    let aof = ind.aof
	    let index = (midiMessage.data[1]+i)%aof.values.length
