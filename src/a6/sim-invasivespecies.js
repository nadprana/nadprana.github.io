...
class InvasiveSpeciesSimulation {
	...
	step() {
		...
		setGrid(this.objects, (x,y) => {
			...
			if (obj == "🌱") {
				...
				// If next to beaver or rabbit, there is a chance of replacing the leaf with a new beaver/rabbit
				neighbors.forEach(n => {
					if (this.objects[n[0]][n[1]] == "🐇") isNextToRabbit = true
					if (this.objects[n[0]][n[1]] == "🦫") isNextToBeaver = true
				})

				if (isNextToBeaver && Math.random() < chanceOfBeaverSpawning) {
					return "🦫"
				}
				else if (isNextToRabbit && Math.random() < chanceOfRabbitSpawning) {
					return "🐇"
				} else {
					return "🌱"
				}
