...
function getLoopingNoise({
  ...
  let theta = 2 * Math.PI * loopLength
...
function setupDrawing0() {
  ...
  function draw(p) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      // Try changing them
      p.ellipse(x, y, x%30, y%30)
      p.noStroke();
      p.fill(hue, 100, pct * 100/1.6)
      p.ellipse(x, y, x%30/3, y%30/3)
  ...
  function draw(p) {
    ...
    for (var i = 0; i < sides; i++) {
      ...
      for (var j = 0; j < dotCount; j++) {
        let offsetX = 150 * p.noise(t + 500, j * .13) - 80
        let offsetY = 200 * p.noise(t + 600, j * .13) - 80
        
        // hue should be between 290-326
        let hue = 36 * i / sides + 290
        p.fill(hue, 100, 30 + 60 * j / dotCount, .5)
        p.circle(offsetX, offsetY - 2, 9)
        p.fill(hue, 100, 90)
        p.circle(offsetX, offsetY, 3)
      }
    ...
    for (var i = 0; i < sides; i++) {
      ...
      let dotCount = 30;
      for (var j = 0; j < dotCount; j++) {
        let offsetX = 150 * p.noise(t + 300, j * .13) - 80
        let offsetY = 200 * p.noise(t + 400, j * .13) - 80
        
        let hue = 36 * i / sides + 320
        p.fill(hue, 100, 30 + 60 * j / dotCount, .5)
        p.circle(offsetX, offsetY - 2, 9)
        p.fill(hue, 100, 90)
        p.circle(offsetX, offsetY, 3)
...
function setupDrawing2() {
  ...
  function draw(p) {
    ...
    for (var numEggs = 0; numEggs < 4; numEggs++) {
      if (numEggs == 1) p.translate(p.width / 2, 0);
      if (numEggs == 2) p.translate(0, p.height / 2);
      if (numEggs == 3) p.translate(-p.width/2, 0);
    ...
    for (var i = 0; i < shapes; i++) {
      ...
      let hue = 25
      let sat = 100
      let light = 10 + (shapes-i)*30
      p.stroke(hue, sat, light-7)
      p.fill(hue, sat, light, 1)
